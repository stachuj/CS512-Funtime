...
MenuResult Menu::Update() {
    ...
    if (IsKeyPressed(KEY_ENTER)) {
        if (type == MenuType::Main) {
            ...
            if (selected == 2) return MenuResult::Exit;
        } else {
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        if (type == MenuType::Main) {
            ...
            if (CheckCollisionPointRec(mouse, {300, 380, 200, 40})) return MenuResult::Exit;
        } else {
...
void Menu::Draw() {
    ...
    if (type == MenuType::Main) {
        ...
        DrawText("Quit",       300, 380, 30, selected == 2 ? GREEN : GRAY);
    } else {
        DrawText("Resume Game", 300, 280, 30, selected == 0 ? GREEN : GRAY);
        DrawText("Game Rules",  300, 330, 30, selected == 1 ? GREEN : GRAY);
        DrawText("Exit Game",   300, 380, 30, selected == 2 ? GREEN : GRAY);
    }
}
